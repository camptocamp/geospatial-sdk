import { EndpointError, WfsEndpoint, WmtsEndpoint } from "@camptocamp/ogc-client";
...
export async function getEndpoint(url: string, type: 'wmts' | 'wfs', map: Map): Promise<WmtsEndpoint | WfsEndpoint> {
  try {
    if (type === 'wfs') {
